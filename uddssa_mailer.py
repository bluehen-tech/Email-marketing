...
# """
# This script is used to send marketing emails to the mailing list.
# It uses the Resend API to send the emails and the OpenAI API to generate the subject and body of the email.
# It uses the DeepSeek API to generate the subject and body of the email.
# It uses the OpenAI API to generate the subject and body of the email.
# It uses the OpenAI API to generate the subject and body of the email.
...
#         for row in reader:
#             name = (row.get("name") or row.get("Name") or row.get(" full_name") or row.get("full_name") or "").strip()
#             email = (row.get("email") or row.get("email ") or row.get(" email")or row.get("Email") or row.get("email address") or row.get("email address ") or "").strip()
#             if email:
#                 recipients.append({"name": name, "email": email})
#     return recipients
...
# ) -> tuple[str, str]:
#     """
#     Provider-aware AI helper that returns (subject, body) where output MUST be JSON.

#     provider:
#       - "openai"   -> OpenAI Responses API
#       - "deepseek" -> DeepSeek Chat Completions API (OpenAI-compatible style)
#     mode:
#       - "ai_draft"  -> create from prompt
#       - "ai_polish" -> improve provided draft_subject/draft_body
#     """
...
#     # System rules: force JSON-only output
#     system_rules = f"""
# You are helping draft an email for a university data science student association (UD-DSSA).
# Tone: {tone}.
# Requirements:
...
#     # ------------------------------------------------------------
#     # Extract raw text (provider-specific)
#     # ------------------------------------------------------------
#     raw = ""
...
#     # --- Service mode: require Resend creds from env/.env ---
#     resend_api_key = os.getenv("RESEND_API_KEY")
#     if not resend_api_key:
#         die("Missing RESEND_API_KEY environment variable (set it in .env).")
